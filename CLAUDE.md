...
- **Net verdict:** the simple rule **stands as a deployable standalone strategy** on the 5-asset sweet spot (stable across a wide parameter region, net of real spreads); the only honest improvement levers are **diversification (already saturated at the curated 5)** and the **vol-target dial (a risk/return choice, not free alpha)** — risk overlays are a NULL. The contribution is "**when the simple rule is enough**": establishing robustness + which improvements are real vs overfit, not an alpha claim. **Downstream (separate plan):** eToro standalone-rule engine + real-price backtest + journal paper + Greek business report.
