...
### paper6 — the rule as a STANDALONE strategy (research harness, 2026-06-10)

paper6 flips the framing: across paper4/5 the fixed rule (banded vol-targeted TSMOM) was always the *baseline* to beat; here it is the **protagonist** — is it robust enough to deploy standalone, and how to improve it *without overfitting*. Built to the `ai-trading` standard on the **5-asset sweet spot** (SPY/TLT/GLD/BTC-USD/UUP), deep Yahoo, leak-free, net of **real per-asset spreads** (BTC 31 bps, ETFs 2-4). `paper6/code/`: `rule.py` (the rule as ONE pure function — single source of truth for harness + future engine), `data.py`/`eval.py` (reuse paper4 `costs`/`metrics` + paper5 `band_eval`/`crypto_data` via `_paths`), `robustness.py`/`overlays.py`/`basket.py`/`sizing_dial.py` + run-drivers, 21 offline tests. Spec/plan: `docs/superpowers/specs|plans/2026-06-10-paper6-rule-standalone-*`. Four pre-registered axes:
- **Axis 1 — robustness / anti-overfit (`run_robustness.py`, `fig_robustness.png`):** swept lookback×band×target_vol; base config picked by `robust_pick` = **center of the stable plateau (median-of-neighbourhood), NOT the argmax** (the argmax `lookback=126, band=0.6, tv=0.2` IR 1.07 is the overfit spike we reject). **Locked base: `lookback=252, band=0.15, target_vol=0.15, smooth_span=5`** (net-IR **0.82**). Key finding: the **no-trade band is a turnover DIAL, not an IR-optimum** — at the base there is a broad high-IR band plateau ~0.15-0.40 (IR 0.82-0.90, gentle peak ~0.30) that then decays into a thin-sample noisy tail; we **cap band at the conservative knee 0.15** rather than chase the tail (consistent with paper5's "band is THE turnover lever, break-even ∝ 1/turnover"). lookback=252 = the classic 12-mo TSMOM standard.
- **Axis 2 — risk overlays (`run_overlays.py`): robust NULL.** Pre-registered gate = cut maxDD ≥20% without losing >0.1 net-IR. Base IR 0.82 / maxDD −14.77%; **+drawdown-control 0.81/−14.28%, +VIX-gate 0.78/−14.89%, +BOCPD-brake 0.83/−14.64% — all NULL** (none cuts maxDD by the threshold; VIX even costs IR by gating off recoveries). Honest finding: **on a well-diversified vol-targeted basket the risk overlays add nothing on top of diversification + vol-targeting** — the basket is already well-behaved (matches paper4's "BOCPD reduces drawdown not alpha", here even the drawdown gain is marginal). 2022 durability is reported separately, not enforced in the verdict (moot — all NULL).
- **Axis 3 — basket / diversification (`run_basket.py`): 5 is saturation, now MEASURED.** Greedy ENB-maximizing selection over a 9-asset pool **independently rediscovers the exact hand-picked 5** (SPY/GLD/BTC/TLT/UUP, ENB **4.3**, IR 0.82, maxDD −14.77%). Going narrower (3 → IR 0.95 but **maxDD −21.5%**, concentration buys return with risk) or wider (7 → ENB 4.5 but IR drops to **0.71**, HYG/SLV dilute the edge) both lose. So **"diversity saturates ~5 well-chosen classes" is confirmed by an objective selector**, not assumed — the sweet spot is the IR/maxDD knee.
- **Axis 4 — sizing / capital dial (`run_sizing.py`): one clean, safe knob.** vol-target presets on €10k (deep-Yahoo OOS over the BTC-limited common window ~2015-2026, per-asset-spread net): **conservative 0.10 → €20.4k, maxDD −10.4%; balanced 0.15 → €25.7k, −14.8%; aggressive 0.20 → €32.0k, −19.7%.** Net-IR is **flat ~0.82-0.86** across presets (the dial scales exposure, not the per-unit-risk edge) while EUR and maxDD scale together monotonically — exactly the paper4/5 "vol-target is the risk/profit dial", now packaged as three end-user presets. (EUR uses the same per-asset-spread cost model as the IR column.)
- **No-BTC deep run (`run_no_btc.py`, 4-asset macro SPY/TLT/GLD/UUP, 2007-2026 incl. 2008) — the edge is BTC-dependent, but the rule is crisis-durable.** Dropping BTC extends the window to 19y (the 5-asset is BTC-limited to ~2014) and the base rule **collapses to net-IR 0.33 (NW-t 1.42 = NOT significant), maxDD −27.9%, ann 2.4%** (vs 0.82 with BTC). So **most of the 5-asset alpha comes from BTC's high-alpha crypto trend**; the macro 4 supply diversification + crisis ballast, not alpha — exactly the paper4/ETF-zoo low-alpha-ETF regime (rule ~0.33-0.56). **BUT the rule IS crisis-durable on the macro basket: positive in all three crises — 2008 +0.21, 2020 +1.39, 2022 +0.06** (the durability the BTC-limited basket cannot test). Year-by-year is very noisy (−1.88 in 2016 to +2.74 in 2024). Presets €10k→€14k/€15.3k/€16.9k (weak, low-alpha regime).
- **Overlays are BASKET-RISK-DEPENDENT, not universally NULL (the no-BTC ablation refines Axis 2).** On the deep-drawdown macro basket (−27.9%, 2008-inclusive), re-running the three overlays: **drawdown-control PASSES** (maxDD −27.9%→**−20.9%**, −25%, losing only 0.04 IR, all three crisis years preserved) — the FIRST overlay to pass anywhere, because here the drawdown is deep enough to be worth cutting (on the shallow −14.8% 5-asset it had no work to do). **BOCPD stays NULL even here** (−27.9%→−27.7%, ~no effect) — its regime-brake is too diffuse to matter; confirms paper4 "BOCPD reduces drawdown not alpha", here not even drawdown. **VIX-gate is a TRAP: it flips 2008 +0.21→−0.20 (and 2022 +0.06→−0.04)** because it de-risks when VIX>30 — but in a crash the trend-rule is SHORT and profiting, so gating off kills the profitable short exactly when protection seems wanted. Lesson: **the only overlay that earns its keep is plain drawdown-control, and only when the base drawdown is deep; BOCPD/VIX do not help (VIX backfires).**
- **Net verdict:** the simple rule **stands as a deployable standalone strategy** on the 5-asset sweet spot (stable across a wide parameter region, net of real spreads), but its **strength is regime/universe-dependent and largely BTC-driven** — on a macro-only basket it is durable-but-weak (IR 0.33, low-alpha ETF regime). The only honest improvement levers are **diversification (already saturated at the curated 5)** and the **vol-target dial (a risk/return choice, not free alpha)**; risk overlays are NULL on the well-behaved 5-asset, and only **drawdown-control** earns its keep on a deep-drawdown basket (**BOCPD NULL everywhere, VIX-gate backfires in crises**). The contribution is "**when the simple rule is enough**": establishing robustness + which improvements are real vs overfit, not an alpha claim. **Downstream (separate plan):** eToro standalone-rule engine + real-price backtest + journal paper + Greek business report.
