...
- **Axis 4 — sizing / capital dial (`run_sizing.py`): one clean, safe knob.** vol-target presets on €10k (2.6y): **conservative 0.10 → €19.5k, maxDD −10.4%; balanced 0.15 → €24.3k, −14.8%; aggressive 0.20 → €30.2k, −19.7%.** Net-IR is **flat ~0.82-0.86** across presets (the dial scales exposure, not the per-unit-risk edge) while EUR and maxDD scale together monotonically — exactly the paper4/5 "vol-target is the risk/profit dial", now packaged as three end-user presets.
